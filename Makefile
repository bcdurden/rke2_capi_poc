...
VSPHERE_TLS_THUMBPRINT 		:= $(shell echo | openssl s_client -connect vcsa.sienarfleet.systems:443 -prexit 2>/dev/null | openssl x509 -fingerprint -noout -sha1 | cut -d= -f2-)
...
test:
	...
	$(call colorecho,"VSPHERE_TLS_THUMBPRINT=$(VSPHERE_TLS_THUMBPRINT)", 6)
